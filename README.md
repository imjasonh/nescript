...
| [`jumpjet.ne`](examples/jumpjet.ne) | **Side-scrolling shooter** — a port of the 1990 DOS game JumpJet (Monte Variakojis / Montsoft). You pilot a Harrier-style VTOL jet, shoot enemy planes with missiles (A button) in the direction you're facing, and bomb tanks on the ground (B button) with gravity-pulled bombs. D-pad changes altitude and facing direction. Split across `examples/jumpjet/*.ne` with a one-block Tileset (alphabet + digits + jet R/L + plane R/L + tank + missile R/L + bomb + explosion + heart + clouds), a static sky-and-ground background, sprite-driven world motion (clouds and enemies pan opposite the jet's heading without any scroll trick), three plane-altitude bands, two ground tanks with rolling movement, AABB collision for missile-vs-plane and bomb-vs-tank, plane-vs-jet damage that ticks down a 3-life pool, and a Title → Playing → GameOver state machine. The headless harness has no input, so an autopilot oscillates altitude on a triangular `frame_tick` wave, flips heading every 128 frames, auto-fires every 32 frames, and auto-bombs every 64 frames — the captured frame at frame 180 shows live missile / bomb action with a non-zero score. |
